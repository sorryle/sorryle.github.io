...
4. 只对前若干行的前若干个元素赋初值，此时所有未赋初值的数组元素均获得零值。 例如：`char a[2][3]={{'1','2'}};`
   ...
   - ![image-20220820154441018](/../assets/images/2021/2021-07-27-code_design/image-20220820154441018.png)
...
6. 注意：允许给出的初值不是列长度的整数倍。此时，行长度=初值个数整除列长度后再加1。例如：`float a[][3]={1,2,3,4,5,6,7};` 则$省略的行长度=7/3+1=2+1=3$
   ...
   - ![image-20220820154954168](/../assets/images/2021/2021-07-27-code_design/image-20220820154954168.png)
...
7. 若分行给所有行的前若干个元素赋初值，行长度可以省略。例如：`float a[][3]={{1},{4,5}};` 系统自动认为数组`a[][3]`的行长度是2
   ...
   78.60000,77.60000,70.38000,73.00000,
...
1. 字符数组是存放字符型数据的数组，每个数组元素存放一个字符。
2. 字符串是若干个字符组成的，其最后一个字符是字符串的结束标识符`'\0'`。
3. C语言中没有字符串变量，一个字符串可以用一个一维字符数组存放；多个字符串用多个一维数组存放，可以用一个二维字符数组存放，每行存放一个字符串。



**字符数组的定义和初始化**

1.  一维字符数组定义：`char 数组名[数组长度];`  `char c[10];`
...
3. 用字符初始化字符数组。  
   ...
   - ![image-20220820172351500](/../assets/images/2021/2021-07-27-code_design/image-20220820172351500.png)

   - ```c
      char str[8]={'p', 'r', 'o', 'g', 'r', 'a', 'm, '\0'};
     ```

   - ![image-20220820173324517](/../assets/images/2021/2021-07-27-code_design/image-20220820173324517.png)

4. 用字符串初始化字符数组   `char str[8]={"program"};` 或 `char str[8]="program";`

   - ![image-20220820173436586](/../assets/images/2021/2021-07-27-code_design/image-20220820173436586.png)
...
5. 用ASCII码初始化数组  
   ...
   - ![image-20220820173533031](/../assets/images/2021/2021-07-27-code_design/image-20220820173533031.png)

6. 初始化时如果只提供了部分元素的值，未提供初值的元素自动赋值为`\0`。 

   - ```c
     char str[8]={'p','r','o','g','r','a','m'};
     ```

   - ![image-20220820173730813](/../assets/images/2021/2021-07-27-code_design/image-20220820173730813.png)

   - ```c
     char str3[10]={"program"}; 
     ```

   - ![image-20220820173821203](/../assets/images/2021/2021-07-27-code_design/image-20220820173821203.png)

   - 上面字符数组str3, 其数组长度是10，但里面存放的字符串的长度是7。

7. 所有元素均赋初值可省略数组长度。

   - ```c
     char str[]={'p','r','o','g','r','a','m'}; //  数组长度为7
     ```

   - ![image-20220820174038940](/../assets/images/2021/2021-07-27-code_design/image-20220820174038940.png)

   - ```c
     char str[]={"program"};    // 数组长度为8
     ```

   - ![image-20220820174130077](/../assets/images/2021/2021-07-27-code_design/image-20220820174130077.png)



**字符二维数组定义和初始化**

【格式】`char 数组名[行长度][列长度]`   `char s[5][6];`

【初始化】

1. 用字符初始化二维字符数组
   - ```c
     char s[2][10]={{'c','0', 'm','p','u','t','e','r'}, {'s', 'c', 'i', 'e', 'n', 'c', 'e'}};
     ```
   
   - ![image-20220820174604265](/../assets/images/2021/2021-07-27-code_design/image-20220820174604265.png)
2. 用字符串初始化二维字符数组
   - ```c
     char s[2][10]={"computer", "science"}
     ```
   
   - ![image-20220820175053476](/../assets/images/2021/2021-07-27-code_design/image-20220820175053476.png)
...
1. 使用`scanf("%c", 一维字符数组元素)`逐个字符输入   

   - ```c
     #include <stdio.h>
     void main(){
     	int i; char s[5];
         for(i=0; i<5; i++) sacnf("%c", s[i]);
     }
     ```

2. 使用`scanf("%s", 一维字符数组名)`整个字符串输入 `sacnf(”%s“, s);`

3. 使用`gets(一维字符数组名)` 整个字符串输入`gets(s);`



**二维字符数组的输入**

1. 二维字符数组的每一行可以看作一个一维字符数组

2. 使用`sacnf("%s", 一维字符数组名)`逐个字符串输入

   - ```c
     #include <stdio.h>
     void main(){
      int i; char s[3][5];
      for(i=0; i<3; i++)sacnf("%s", s[i]);
     }
     ```

3. 使用`gets(一维字符数组名)`逐个字符串输入
...
2. 使用`printf("%s", 一维字符数组名)`  整个字符串输出 `printf("%s", s);`

3. 使用`puts(一维字符数组名)` 整个字符串输出`puts(s);`



**二维字符数组的输出**

1. 使用`printf("%c", 数组元素)` 逐个字符输出
...
2. 使用`printf("%s", 一维数组数组名)`逐个字符串输出  `for(i=0;i<3;i++)printf(s[i]);`

3. 使用`puts(一维字符数组)`逐个字符串输出  `for(i=0; i<3; i++) puts(s[i]);`
...
【功能】从键盘接收一个字符串（仅以回车换行结束标记），存入指定的字符数组中。

【返回值】字符数组的首地址。

【说明】对于`gets()`函数，只有"回车换行符"才看成输入字符串的结束，“空格符”看成字符串的一部分。对于`scanf("%s")`函数,“回车换行符”或"空格符"都看成输入字符串的结束。

例子：使用`gets()`函数和`scanf()`函数输入字符串的不同之处举例。
...
输出  差异:gets输入的空格也当成字符处理       scanf输入的空格当成字符结束
123 231   
1235312
5214
213
...
【功能】输出字符数组中存放的字符串，其中结束标记转换称回车换行符输出。

【说明】`puts()`函数和使用`"%s"`格式的`printf()`函数都可以输出字符串，但输出时有区别：对于`printf()`函数，`"字符串结束标记符"`不输出；对于`puts()`函数, 首个`”字符串结束标记符"转换`成`"回车换换行符"`输出

例子：使用`puts()`函数和`printf()`函数输出字符串不同之处举例。
...
【参数】字符串1和字符串2时字串常量或已存放字符串的字符串数组名。

【功能】

1. 若字符串1等于字符串2，返回值为0；
2. 若字符串1小于字符串2，返回值为负整数；
3. 若字符串1大于字符串2，返回值为正整数；

【说明】

1. 不能使用关系运算符“==”比较两个字符串，只能调用`strcmp()`函数进行处理。
2. 比较的字符串的ASCII码值    `net`比`Net`大
...
#include <stdio.h>
void main(){
	char str1[]="net", str2[]="Net";
    printf("%d", strcmp(str1, str2)>0);   //str1是否比str2大
    printf("%d", strcmp(str1, str2)<0);   //str1是否比str2小
    printf("%d", strcmp(str1, str2)==0);  //str1是否和str2相等
...
【格式】`strcpy(字符数组名, 字符串, 整型表达式)`

【参数】

1. **字符数组名**：已定义的字符数组名；
2. **字符串**：字符串常量或已存放字符串的字符数组名;
3. **整型表达式**：任何整型表达式。本参数可以省略。

【功能】

- 将“字符串”的前“整型表达式”个字符组成的新的字符串存入“字符数组”中
- 省略“整型表达式”时。则将整个“字符串”存入字符数组中。

【返回值】字符数组的首地址。

【说明】

1. 字符数组的长度要足够大，以便容纳赋值过来字符串。复制时，连同字符串结束标志`\0`一起复制
2. 不能用复制运算符`=`将一个字符串直接复制给一个字符数组，只能调用`strcpy()`函数处理。
